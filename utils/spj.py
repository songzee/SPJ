...
class SPJ(object):
    ...
    def __init__(self, config):
        ...
        # LSTM Layer
        feature_inputs = tf.tile(input=feature_inputs,multiples=[1,self.config.num_steps,1])
        lstm_inputs = tf.concat(values=[feature_inputs, embedding_inputs],axis=2) # stack --> shape=[-1, None, 512*2]
        print(lstm_inputs.get_shape().as_list())
